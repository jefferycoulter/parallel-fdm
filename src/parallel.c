...
void GetInput(Subdomain *sd, int n_proc, int rank)
{
    ...
    //sd->n_proc_dim2D[0] = sd->n_proc_dim2D[1] = 1;
    //sd->n_proc_dim3D[0] = sd->n_proc_dim3D[1] = sd->n_proc_dim3D[2] = 1;

    sd->grid_g[0] = 200; // columns
    sd->grid_g[1] = 200; // rows
    sd->grid_g[2] = 1; // depth
...
void SplitProcessorsAlongDims(Subdomain *sd)
{
    ...
    if (sd->n_dims == 2)
    {
        ...
        sd->n_proc_dim2D[0] = sd->n_proc; // along columns
        sd->n_proc_dim2D[1] = 1; // along rows
    }
    else if (sd->n_dims == 3)
    {
        // this would divide processors evenly among dimensions
        ...
        sd->n_proc_dim3D[0] = 1; // along columns
        sd->n_proc_dim3D[1] = sd->n_proc; // along rows
        sd->n_proc_dim3D[2] = 1; // along depth
    }
    else
    {
...
void DetermineSubdomainGridBounds(Subdomain *sd)
{
    if (sd->n_dims == 2)
    {
        ...
        sd->coords[0] = floor(sd->rank / sd->n_proc_dim2D[0]);
        sd->coords[1] = sd->rank - sd->coords[1] * sd->n_proc_dim2D[1];
        sd->coords[2] = 0;
        ...
        for (int n = 0; n < sd->n_dims; n++)
        {
            ...
            {
                // corresponds to col_start, row_start, depth_start
                sd->bounds_l[2 * n] = 0;
                // corresponds to col_end, row_end, depth_end
...
void CreateSubdomainType(Subdomain *sd)
{
    ...
    else if (sd->n_dims == 3)
    {
        ...
        MPI_Type_create_resized(subdomain, 0, sd->grid_l[2] * sizeof(int), &(*sd).subdomain_type);
        MPI_Type_commit(&(*sd).subdomain_type);
    }
    else
    {
...
void SetupCollectSubdomainData(Subdomain *sd)
{
    if (sd->n_dims == 2)
    {
        ...
        int disp = 0;
        int r = 0, c = 0, d = 0;
        for (int i = 0; i < sd->n_proc_dim2D[0] * sd->n_proc_dim2D[1]; i ++)
        {
            sd->displs[r * sd->n_proc_dim2D[1] + c] = disp;
            disp += 1;
            if (i % sd->n_proc_dim2D[1] == 0) { disp += (sd->grid_l[0] - 1) * sd->n_proc_dim2D[0]; }
    ...
    else if (sd->n_dims == 3)
    {
        ...
        for (int i = 0; i < sd->n_proc_dim3D[0]; i++)
        {
            ...
            disp += ((sd->grid_l[1])  * (sd->grid_l[2]) - 1) * sd->n_proc_dim3D[2];
        }
    }
    else
    {
...
void CoordShift(Subdomain *sd, float rad)
{
    ...
    for (int i = 0; i < sd->grid_l[0] * sd->grid_l[1] * sd->grid_l[2]; i++)
    {
        if (sd->n_dims == 2)
        {
            ...
            local_id = (r - sd->bounds_l[0]) * sd->grid_l[0] + (c - sd->bounds_l[3]);
            r_shifted = pow(c - (sd->grid_g[0] / 2), 2.0) + pow(r - (sd->grid_g[1] / 2), 2.0);
        }
        else if (sd->n_dims == 3)
        {
            local_id = ((d - sd->bounds_l[4]) * sd->grid_l[1] + (r - sd->bounds_l[0])) * sd->grid_l[0] + (c - sd->bounds_l[3]);
            r_shifted = pow(c - (sd->grid_g[0] / 2), 2.0) + pow(r - (sd->grid_g[1] / 2), 2.0) + pow(d - (sd->grid_g[2] / 2), 2.0);
        }

        if (r_shifted <= rad*rad) { sd->shape_now[local_id] = 1; } else { sd->shape_now[local_id] = 0; }   
        Increment(i, r, c, d)
    }
